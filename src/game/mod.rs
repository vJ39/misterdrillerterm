...
impl Game {
    ...
    /// 何もしない(通常の1人プレイに影響を残さない)。
    fn add_attack_power(&mut self, blocks: usize) {
    ...
    fn try_start_incoming_wave(&mut self) -> usize {
    ...
    fn tick_incoming_rocks(&mut self, delta_ms: u32, events: &mut Vec<GameEvent>) {
        if self.incoming_rocks.is_empty() {
            return;
        }
...
mod tests {
    ...
    fn attack_rules_game(seed: u64) -> Game {
        let mut game = Game::new(seed);
        clear_board(&mut game);
        game.set_bomb_spawn_rate_percent(0);
