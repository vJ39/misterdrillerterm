...
impl Game {
    ...
    /// 直近で到達済みのチェックポイント区切り番号(TERM独自拡張。#178/#190)。
    /// 描画側(render.rs)が、まだ掘り抜いていないチェックポイントより先のゾーンを
    /// 隠す判定に使う(#197。ユーザー指摘: 「この地面の上を掘ったら次の100mゾーンに
    /// 進めるようにしたい。それまで次の100mゾーンはブロック配置しない」)。
