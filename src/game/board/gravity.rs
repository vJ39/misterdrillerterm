//! 重力落下・支持判定ロジック(spec.md 4章・5章)。盤面を連結グループ(塊)に分割し、
//! 塊単位の支持判定・揺れ(4.3)・1マス落下・着地時の自動消滅(4.5・4.9)を扱う。
...
pub struct GravityState {
    ...
    /// `board::generate`のテストがreroll検証用に直接書き込むため`pub(crate)`
    /// (モジュール分割前は同一モジュールで素の非公開フィールドとして参照できていた)。
    pub(crate) shaking_cells: HashSet<(usize, usize)>,
}
...
pub(crate) fn collect_fall_groups(board: &Board) -> Vec<Vec<(usize, usize)>> {
    let depth_rows = board.depth_rows();
