...
/// ミス時に「潰れた」見た目(平たいスプライト)へ切り替える長さ(ms)。実際にはミスの
/// 直後から`CRUSH_ASCEND_MS`の「天に召される」演出が続くため、その間もこの見た目の
/// ままになる。
pub const CRUSH_FLASH_MS: u64 = 400;

/// ミス時の「天に召される」演出の長さ(ms)。この間プレイヤー自身の入力・自由落下・
/// 酸素減少を凍結し、終了時に死亡地点の3列クリア・ライフ減算を行う。ライフが残って
/// いれば酸素を全回復してその場で復活し、最後のライフだった場合はここでGameOverへ
/// 進む(#257)。
...
///
/// これは基準tick(`FALL_TICK_MS`=150ms)での長さで、実際の長さは実効tickに比例して
/// スケールする(`Game::vanish_flash_duration_ms`)。落下tick間隔を上げる(遅くする)と
/// 1tickぶんの落下補間が長くなるため、固定値のままだと落下が終わる前に演出が尽きて
/// しまい、落下中のブロックが空中で消えていた(#234)。
pub const BLOCK_VANISH_FLASH_MS: u64 = 200;

/// 消滅フラッシュの長さの下限(ms)。3フレーム分(白→背景のフェードが視認できる最低限)。
/// 落下tickを最速まで上げると比例値がこれを下回るため、下限で止める。
pub const BLOCK_VANISH_FLASH_MIN_MS: u64 = FRAME_INTERVAL_MS * 3;
...
/// 爆発前に必ず視覚的な予兆(点滅)を出す設計方針を踏襲する。設定で変更可能な既定値。
pub const BOMB_FUSE_MS: u32 = 5000;

/// ボム爆発までの時間設定の下限(ms)。`BOMB_DANGER_MS`(危険域突入の閾値)より必ず
/// 大きくする。
pub const BOMB_FUSE_MS_MIN: u32 = 2000;
/// ボム爆発までの時間設定の上限(ms)。
pub const BOMB_FUSE_MS_MAX: u32 = 10000;
/// ボム爆発までの時間設定の調整刻み幅(ms)。
pub const BOMB_FUSE_MS_STEP: u32 = 500;
...
/// 経路上のアイテムブロック(頭上クリア/スター化)への加点の上限。どちらも進路を
/// 大きく拓くため、先読み範囲いっぱい(14行)に迫る価値を与える。2色化は効果が進路と
/// 無関係なため加点しない。
///
/// スター化(K)の実際の加点は、その効果が今の盤面でどれだけ役に立つかに比例させた値に
/// なる(TERM独自拡張。#229 F4)。一律加点では「岩が1つも無いのにスター化へ寄り道する」が
/// 起きていた。頭上クリア(R)を同じように連動させる案は実測で成績が落ちたため、上限値の
/// ままにしてある。
pub const AUTOPLAY_SCORE_ITEM_BONUS: f32 = 10.0;

/// スター化アイテム(K)の価値が上限に達する、先読み範囲内の岩の数(TERM独自拡張。#229)。
/// 岩0個なら加点0、この数以上で`AUTOPLAY_SCORE_ITEM_BONUS`いっぱい。岩1個の代償
/// (`rock_cost_rows`、既定で30〜70行)を考えると、数個まとめてスターに変えられるなら
/// 寄り道する価値がある。
pub const AUTOPLAY_ITEM_STARIFY_ROCK_SATURATION: usize = 6;
...
/// 上と同じ必要量の上限(行数)。横移動を遅く・ブロック落下を速く設定すると必要量が
/// 際限なく伸び、物理的には通れる列まで軒並み候補から外れて岩を割るしかなくなる
/// (実測: 横移動120ms・落下75msの設定で6.1〜8.8行を要求していた。#225)。
pub const AUTOPLAY_THREAT_MAX_SLACK_ROWS: f32 = 5.0;

/// 岩を割る酸素が払えないとき、隣の列が落ちてくる塊で塞がっているだけなら着地を待つ。
/// その待ち時間の上限(ms)。自然減少は2〜5%/秒なので、1.5秒待つ方が岩1個の20%より
/// ずっと安い(#225)。
pub const AUTOPLAY_WAIT_FOR_THREAT_MAX_MS: u64 = 1500;

/// 岩を割って買った前進がこの回数続いたら、手詰まり段階(escalation)を最低1へ引き上げる
/// (#225)。岩で1行進むと停滞ベースの判定は「前進」とみなして段階を0へ戻すため、
/// 横方向の岩掘りが解禁される段階(2)へ実質到達できなかった(実測で全岩破壊判断のうち
/// escalation1以上で起きたものは0件)。
pub const AUTOPLAY_ROCK_STREAK_ESCALATE: u8 = 2;
