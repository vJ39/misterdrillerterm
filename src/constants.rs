...
/// 最大個数(TERM独自拡張。ユーザー指摘: 「各種アイテムのキャラより上の位置には
/// 最大ぞれぞれ10個までとする」)。
///
/// 当初(#113)は「盤面全体(1000行)で生涯10個」という仕様だったが、事故: 配分率を
/// 高く(300%)設定すると深度40〜50m台で使い切ってしまい、残り900m以上まったく
/// 出現しなくなった(#210)。ユーザー指摘「自分より少し先の範囲内で常に最大10個に
/// なるよう補充してほしい」を受け、`ITEM_WINDOW_AHEAD_ROWS`ぶんのプレイヤー前方の
/// 窓を単位とする上限に変更した(#211)。窓は深度が進むにつれてスライドし、
/// プレイヤーより後ろに落ちた既存アイテムは(回収済みかどうかを問わず)窓の外に
/// なるため、その分だけ前方に新規出現の余地が生まれる。
pub const ITEM_MAX_COUNT_ON_BOARD: usize = 10;

/// アイテムブロック3種を「プレイヤーより少し先の範囲内で常に`ITEM_MAX_COUNT_ON_BOARD`
/// 個になるよう補充する」窓の広さ(行数、TERM独自拡張。#210/#211)。プレイヤーの
/// 現在行からこの行数だけ先までを対象に既存個数を数え、上限未満なら不足分を
/// 追加で抽選する。値が大きいほど窓内の密度は薄まる(同じ上限をより広い範囲に
/// 分散させるため)。
