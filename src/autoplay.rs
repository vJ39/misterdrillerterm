...
//! これは各案を評価した時点の値で、その後の最終構成では完走7/32・平均879m・
//! 5.06回/走まで動いている。以後の数値もその都度の基準点であり、単調に改善し
//! 続けているわけではない点に注意)。
//!
...
//!
//! ユーザー提案の次の2案も実測A/Bで検証したが、**採らなかった**(#231)。
//!
//! - **頭上で揺れているブロックを先制的に掘って消す**: 「1発で消せて、消した後に
//!   その列の上に別の不安定塊が残らない」場面は既定コースで32シード中わずか
//!   4エピソードしかなく、押し潰し死53件中この機会が直前にあったものは0件だった。
//!   試しても効果は中立〜微悪化(harshで死亡1.84→1.94回/走)
//! - **岩を割る代わりにStarifyアイテムを取りに行って節約する**: 1回の見送り
//!   (通過)で浮く岩は0.23〜0.40個(酸素4.6〜8%相当)で、寄り道のコスト(横数列・
//!   自然減少)と同じ桁でしかなく、常に得とは言えなかった。価値の重み・候補範囲の
//!   広げ方を変えた6バリアントを試したが、既定コースは誤差範囲、harsh設定では
//!   全バリアントで完走が32→28〜29に悪化した
//!
//! #231で見つかった「スターブロックを恒久固体として扱っている疑い」は実測で
//! **否定された**(#232)。押し潰し死のうちスター消滅が起点だったものは既定1000mで
//! 0/53件・harshでも2/28件しかなく、その2件も「揺れ中の残り時間を半分と見積もる」
//! 既存ロジックの誤差が原因で、スターを固体扱いすること自体は問題ではなかった。
//! 修正案(消滅までの残り時間を猶予に足す/スターを支えに数えない)は前者が中立
//! (harsh死亡1.84→1.91回/走・既定1000m死亡5.06→5.03回/走)、後者は悪化
//! (harsh完走32→29/32)で、どちらも採らない。揺れ中の塊の残り時間を物理から正確に
//! 取る案はharshの押し潰しを28→2件まで減らせるが酸素切れへ振り替わり、既定1000mでは
//! 押し潰しが53→68件へ悪化するため、深度依存のマージン設計込みで別途検討する。
//!
...
//! 左右に往復するのを防ぐ。
//!
//! # 「AIRの取り逃し」の自由落下コミット問題を調査したが見送った(#236)
//!
//! 未接地(自由落下中)は横移動が効かないため、`choose_target_column`は
//! `lateral_path_is_open`経由で現在列以外を軒並み到達不能扱いにする。目的列へ
//! 歩いている途中で経路上の別の列の床が抜けてたまたま1行落ちただけでも、この呼び出し
//! 自体が副作用で`target_col`をNoneへ押し戻していた。着地後は最初から採点し直しに
//! なるため、下にまだあるはずのAIRへ向けて歩いていた途中でも通り過ぎることがあった
//! (計測上の`missed_air_while_low(only_targeted)`。目的列に一度なったのに取れずに
//! 通過した回数)。
//!
//! 中断中は`target_col`を保持し着地後に歩きの続きを再開する案を実測したところ、
//! 1000mコースの目的列の取り逃しは113→59回(1走あたり約半分)に減ったが、押し潰し死が
//! 1.66→1.81回/走へ増え、`soak_full_course_within_life_budget`が確認している上限
//! (#225修正前の実測1.78回/走を上回らないこと)を超えた。中断の長さで足切りする
//! バリアント(1行以内の中断だけ据え置く)も試したが、実際の中断はほぼ常に1行以内
//! だったため効果は変わらなかった(押し潰し1.81回/走のまま)。ヒステリシスを外して
//! 着地の都度まっさらに選び直す安全な変種は押し潰しを1.72回/走まで戻すが、それは
//! `target_col`を保持しない現状と実質同じ選択になるため取り逃しの改善も消える
//! (113回のまま)。つまりこの経路では「AIRを逃さない」と「押し潰されない」が
//! 同じ1つのつまみの両端にあり、両立するバリアントが見つからなかった(#225 F3や
//! #229の却下案と同じ構図)。既存の押し潰し回避の基準は緩めない方針のため、
//! この案は採らず現状維持とする。
//!
//! # 「振ってくるAIR/アイテムを待つ」戦略を検討したが見送った(#239)
//!
//! 頭上のAIR/アイテムは他のブロックと同じ支持判定(`apply_gravity_tick`)に従い、
//! 揺れてから1マスずつ落ちてくる。だが`is_falling_hazard`はAIR/アイテムを脅威にも
//! 待機対象にも数えないため、現行AIはこれを一切見ておらず、浅い深度では掘削の方が
//! AIRの落下より速いため掘り進んで置き去りにし(自列の頭上のAIR/アイテムの46%は
//! チェックポイント地面直前の隙間帯で`purge_checkpoint_zone_debris`により消去される
//! のが最多の結末)、深い場所では自然に追いついて拾えている(1000mコースの取得率34%)。
//!
//! 接地中に自列頭上の価値あるAIR/アイテムの落下を待つプロトタイプ(複数バリアント)を
//! 実測したところ、AIR取得数は+10%増えたが、1000mコースの押し潰し死が全バリアントで
//! 1.72〜1.83→1.84〜2.19回/走へ増え、`soak_full_course_within_life_budget`の回帰基準
//! (1.78回/走)を割った。調整に使っていない64シードでも同じ傾向(押し潰し1.83→2.06回/走)
//! で、酸素切れ死が減ったぶんがそのまま押し潰し死に振り替わるだけだった(#236と同じ
//! 「AIRを逃さない⇔押し潰されない」の構図)。よってこの案も見送り、現状維持とする。
//!
